...
def parse_args():
    ...
    parser.add_argument("data", help="Path to data pandas pickle")
...
def main():
    ...
    # Read in dataframes
    df_data = pd.read_pickle(options.data)
    df_mc = pd.read_pickle(options.mc)
    ...
    jobs = []
    for varname in pd.concat([df_data, df_mc]).index.get_level_values("varname").unique():
        for dataset in df_data.index.get_level_values("parent").unique():
            for cutflow in pd.concat([df_data, df_mc]).index.get_level_values("selection").unique():
                ...
                if not os.path.exists(outdir):
                    os.makedirs(outdir)
                job_cfg["outpath"] = os.path.join(outdir, cfg[varname]["outpath"])
                jobs.append((
                    df_data.loc[(varname, cutflow, dataset),:],
                    df_mc.loc[(varname, cutflow),:],
                    copy.deepcopy(job_cfg),
                ))
